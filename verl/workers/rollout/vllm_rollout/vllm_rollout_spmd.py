    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if not do_sample:
            ...
            kwargs = {
                ...
                "max_tokens": prompts.meta_info.get("max_tokens", self.config.response_length),  # allow longer generation
            }
        elif is_r2q:
            kwargs = {
        ...
        with self.update_sampling_params(**kwargs):
            ...
            response = pad_2d_list_to_length(response, self.pad_token_id, max_length=prompts.meta_info.get("max_tokens", self.config.response_length)).to(
                idx.device
            )
            if self.config.calculate_log_probs:
                rollout_log_probs = pad_2d_list_to_length(
                    rollout_log_probs, -1, max_length=prompts.meta_info.get("max_tokens", self.config.response_length)
                ).to(idx.device)

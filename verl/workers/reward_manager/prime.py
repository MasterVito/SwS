...
from verl.utils.reward_score import default_compute_score, default_compute_score_light
...
async def parallel_compute_score_async(
    ...
    with ProcessPoolExecutor(max_workers=num_processes) as executor:
        ...
        try:
            ...
            tasks_async = [
                single_compute_score(evaluation_func, c, r, t, ei, executor, timeout=300.)
...
def run_reward_scoring(evaluation_func, completions, references, tasks, extra_info=None, num_processes=os.cpu_count()//2):
...
class PrimeRewardManager(AbstractRewardManager):
    ...
    def __init__(
        ...
        light_eval: bool = False,
    ) -> None:
        self.tokenizer = tokenizer
        self.num_examine = num_examine  # the number of batches of decoded responses to print to the console
        self.compute_score = compute_score or default_compute_score
        self.reward_fn_key = reward_fn_key
        self.light_eval = light_eval
        if self.light_eval:
            self.compute_score = default_compute_score_light
            print(">>> Using light eval for reward computing, only 'init_check', 'math_verify', and 'math_dapo'.")

...
from ..math import compute_score as init_compute_score
...
# def compute_score_light_weight(model_output: str, ground_truth: str, answer: str = "") -> bool:
#     model_output = str(model_output)[-300:]
#     ground_truth = str(ground_truth)

#     is_matched, extracted_model_output = match_answer(model_output)
#     format_correctness = "\\boxed{" in model_output and "}" in model_output.split("\\boxed{")[-1]
...
def compute_score_light_weight(model_output: str, ground_truth: str, answer: str = "") -> bool:
    model_output = str(model_output)
    # ground_truth = "\\boxed{" + str(ground_truth) + "}"
    ground_truth = str(ground_truth)
    last_model_output = model_output[-300:]

    is_matched, extracted_model_output = match_answer(last_model_output)
    format_correctness = "\\boxed{" in last_model_output and "}" in last_model_output.split("\\boxed{")[-1]

    # No print for easy reading in logging files
    # if random.random() < 0.01:
    #     print(">>> Enabling compute_score_light_weight for checking the effectiveness of light weight evalutor ...")
    ## Return False if \\boxed{ is not in the model output
    # print(init_compute_score(last_model_output, ground_truth), math_dapo.compute_score(last_model_output, ground_truth)['score'] > 0)
    if not format_correctness:
        return False, format_correctness, extracted_model_output
        
    ## Verifier-1: init_compute_score (the lighteval/MATH evalutor here since it is light weighted)
    init_check = init_compute_score(last_model_output, ground_truth)
    if init_check:
        return True, format_correctness, extracted_model_output

    # Verifier-2: DAPO math verifier
    is_correct_dapo_verify = math_dapo.compute_score(last_model_output, ground_truth)['score'] > 0
    if is_correct_dapo_verify:
        return True, format_correctness, extracted_model_output

    ## Verifier-3: math_verify
    # is_correct_math_verify = math_verify.compute_score(original_model_output, ground_truth)
...
def compute_score(model_output: str, ground_truth: str, answer: str = "") -> bool:
    model_output = str(model_output)
    ground_truth = str(ground_truth)
    last_model_output = model_output[-150:]
    ...
    ## Verifier-1: grade_answer (grade simple algebra questions. if succeeded, return; otherwise, proceed to more complex grading)
    if grade_answer(extracted_model_output, ground_truth):
        return True, format_correctness, extracted_model_output
    
    ## Verifier-2: init_compute_score (the lighteval/MATH evalutor here since it is light weighted)
    init_check = init_compute_score(model_output, ground_truth)
    if init_check:
        return True, format_correctness, extracted_model_output
    
    ## Verifier-3: math_verify
    is_correct_math_verify = math_verify.compute_score(last_model_output, ground_truth)
    if is_correct_math_verify:
        return True, format_correctness, extracted_model_output

    ## Verifier-4: DAPO math verifier
    is_correct_dapo_verify = (math_dapo.compute_score(last_model_output, ground_truth)['score'] > 0)
    if is_correct_dapo_verify:
        return True, format_correctness, extracted_model_output

    ## Verifier-5: Math Ruler
    # is_correct_mathruler_verify = mathruler.grade_answer(mathruler.extract_boxed_content(original_model_output), original_ground_truth)
    # if is_correct_mathruler_verify:
    #     return True, format_correctness, extracted_model_output

    ...
    def __init__(self, project_name, experiment_name, default_backend: str | list[str] = "console", config=None):
        ...
        if 'tracking' in default_backend or 'wandb' in default_backend:
            ...
            try:
                ...
                wandb.init(project=project_name, name=experiment_name, config=config, id=id, resume="allow", settings=settings)
            except:
                id = wandb.util.generate_id()
                wandb.init(project=project_name, name=experiment_name, config=config, reinit=True, id=id, resume="allow", settings=settings)
            print(f'wandb project: {project_name}, name: {experiment_name}, config: {config}, reinit: True, id: {experiment_name}')

...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.sas_token = self.get_sas_token_from_yaml(os.path.join(os.getcwd(), "configs/cluster_mount_config.yaml")) 
    ...
    def load_checkpoint(self, local_path: str, hdfs_path: str = None, del_local_after_load=False):
        ...
        current_dir = os.path.join(os.getcwd(), "temp", "Temp_Ckpt_Dir"); os.makedirs(current_dir, exist_ok=True)
        with get_fsdp_state_ctx(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
            if self.should_load_model:
                remote_model_path = os.path.join(local_path, f"model_world_size_{self.world_size}_rank_{self.rank}.pt")
                if "/home/t-xiaoliang/xiao/blob/" in local_path and self.sas_token is not None:
                    blob_path = f"https://zhibinmain.blob.core.windows.net/xiaoliang/{local_path.split('xiao/blob/')[1]}"
                    subprocess.run(["azcopy", "copy", f"{blob_path}/model_world_size_{self.world_size}_rank_{self.rank}.pt{self.sas_token}", current_dir])
                    model_state_dict = torch.load(os.path.join(current_dir, f'model_world_size_{self.world_size}_rank_{self.rank}.pt'), weights_only=False)
                else:
            ...
            if self.should_load_optimizer:
                ...
                if "/home/t-xiaoliang/xiao/blob/" in local_path and self.sas_token is not None:
                    blob_path = f"https://zhibinmain.blob.core.windows.net/xiaoliang/{local_path.split('xiao/blob/')[1]}"
                    subprocess.run(["azcopy", "copy", f"{blob_path}/optim_world_size_{self.world_size}_rank_{self.rank}.pt{self.sas_token}", current_dir])
                    optimizer_state_dict = torch.load(os.path.join(current_dir, f'optim_world_size_{self.world_size}_rank_{self.rank}.pt'), weights_only=False)
                else:
        ...
        if self.should_load_extra:
            ...
            if "/home/t-xiaoliang/xiao/blob/" in local_path and self.sas_token is not None:
                blob_path = f"https://zhibinmain.blob.core.windows.net/xiaoliang/{local_path.split('xiao/blob/')[1]}"
                subprocess.run(["azcopy", "copy", f"{blob_path}/extra_state_world_size_{self.world_size}_rank_{self.rank}.pt{self.sas_token}", current_dir])
                extra_state_dict = torch.load(os.path.join(current_dir, f'extra_state_world_size_{self.world_size}_rank_{self.rank}.pt'), weights_only=False)
            else:
    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        ...
        with warnings.catch_warnings():
            ...
            with get_fsdp_state_ctx(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
                ...
                ### save to local and use azcopy to send to blob
                current_dir = os.path.join(os.getcwd(), "temp", "Temp_Ckpt_Dir"); os.makedirs(current_dir, exist_ok=True)
                blob_path = f"https://zhibinmain.blob.core.windows.net/xiaoliang/{local_path.split('xiao/blob/')[1]}"
                ...
                if self.should_save_model:
                    ...
                    if "/home/t-xiaoliang/xiao/blob/" in local_path and self.sas_token is not None:
                        torch.save(model_state_dict, os.path.join(current_dir, f'model_world_size_{self.world_size}_rank_{self.rank}.pt'))
                    else:
                ...
                if self.should_save_optimizer:
                    ...
                    if "/home/t-xiaoliang/xiao/blob/" in local_path and self.sas_token is not None:
                        torch.save(optimizer_state_dict, os.path.join(current_dir, f'optim_world_size_{self.world_size}_rank_{self.rank}.pt'))
                    else:
                ...
                if self.should_save_extra:
                    ...
                    if "/home/t-xiaoliang/xiao/blob/" in local_path and self.sas_token is not None:
                        torch.save(extra_state_dict, os.path.join(current_dir, f'extra_state_world_size_{self.world_size}_rank_{self.rank}.pt'))

                    else:
                        torch.save(extra_state_dict, extra_path)
                    log_with_rank(f"Saved extra_state to {os.path.abspath(extra_path)}", rank=self.rank, logger=logger)

                if "/home/t-xiaoliang/xiao/blob/" in local_path and self.sas_token is not None and self.should_save_model and self.should_save_optimizer and self.should_save_extra:
                    subprocess.run(["azcopy", "copy", f"{current_dir}/*{self.world_size}_rank_{self.rank}.pt", f"{blob_path}/{self.sas_token}"])
                    os.remove(os.path.join(current_dir, f'model_world_size_{self.world_size}_rank_{self.rank}.pt'))
                    os.remove(os.path.join(current_dir, f'optim_world_size_{self.world_size}_rank_{self.rank}.pt'))
                    os.remove(os.path.join(current_dir, f'extra_state_world_size_{self.world_size}_rank_{self.rank}.pt'))
    ...
    def get_sas_token_from_yaml(self, file_path):
        import yaml
        import os
        try:
            with open(file_path, 'r') as f:
                data = yaml.safe_load(f)
                if data and 'azstorage' in data and 'sas' in data['azstorage']:
                    return data['azstorage']['sas']
                else:
                    return None

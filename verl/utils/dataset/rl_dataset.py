...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        self.math_prompt = " Please reason step by step, and put your final answer within \\boxed{}." if self.config.use_math_prompt else ""
    ...
    def _read_files_and_tokenize(self):
        ...
        if self.config.use_math_prompt:
            self.dataframe = self.dataframe.map(lambda x:{"prompt": [{"content": x["prompt"][0]["content"] + self.math_prompt, 'role': 'user'}]}, num_proc=self.num_workers, desc=f"Adding math prompt")
...
class BufferedDataLoader:
    ...
    def get_from_buffer(self, count, dp_size):
        ...
        samples = self.buffer.buffer_slice(range(0, count))
        self.buffer = self.buffer.buffer_slice(range(count, self.buffer_size()))
        return samples

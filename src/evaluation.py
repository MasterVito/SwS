...
def verify(sample):
    ...
    for pred in sample['prediction']:
        acc_compute_score = compute_score(pred, sample['ref_answer'])[0]
...
def evaluate_and_print(outputs, model_path, additional_str=""):
    ...
    outputs_verify = outputs.map(verify, num_proc=64)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--reason_prompt_path", default='prompts/question_answering.txt', type=str)
    ...
    assert "ref_answer" in dataset.column_names, "You need the 'ref_answer' key when evaluating math"
    
    reason_prompt = open(args.reason_prompt_path, "r").read()
    dataset = dataset.map(lambda x: {'question': reason_prompt.replace(r"{REPLACE}", x['question'])}, num_proc=64)
    tokenizer = AutoTokenizer.from_pretrained(args.model_name_or_path)
    add_generation_prompt = False if "llama" in args.model_name_or_path.lower() else True
    dataset = dataset.map(lambda x: {"vllm_input": tokenizer.apply_chat_template([{"content": x['question'], 'role': 'user'}], add_generation_prompt=add_generation_prompt, tokenize=False)}, num_proc=64)
    ...
    dataset = dataset.map(verify, num_proc=64)
